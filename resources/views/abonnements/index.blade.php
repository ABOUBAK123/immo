...
<div style="background:linear-gradient(135deg,#FFF1F2,#FFE4E6);border:1px solid #FECDD3;border-radius:14px;
    ...
    <div style="flex:1">
        ...
        <div style="font-size:.82rem;color:#BE123C;margin-top:2px">
            Souscrivez à un abonnement mensuel pour accéder à toutes les fonctionnalités ImmoGest.
        </div>
    </div>
</div>
@endif
...
{{-- ── Colonne gauche : statut + historique ───────────────────────────── --}}
<div>
    ...
    <div class="card-immo" style="padding:28px;margin-bottom:20px;
        ...
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid #BBF7D0;
            ...
            <span style="font-size:.78rem;color:#166534">
                ...
                — {{ number_format($prix, 0, ',', ' ') }} {{ $devSymbole }}/mois
            </span>
            @if($jours <= 10)
    ...
    <div class="card-immo" style="padding:24px;margin-bottom:20px">
        <div style="font-size:.85rem;font-weight:800;color:#374151;margin-bottom:16px">
            <i class="bi bi-star-fill me-2" style="color:#F97316"></i>Ce qui est inclus
        </div>
        @php
        $features = [
            ['bi bi-buildings', 'Gestion illimitée de biens', 'Ajoutez autant de biens que vous souhaitez'],
            ['bi bi-people',    'Gestion des locataires',     'Fiches locataires, baux et documents'],
            ['bi bi-wallet2',   'Suivi des paiements',        'Loyers, quittances automatiques, relances'],
            ['bi bi-tools',     'Gestion des interventions',  'Suivi des travaux et maintenances'],
            ['bi bi-bell',      'Notifications automatiques', 'SMS, WhatsApp et email aux locataires'],
            ['bi bi-robot',     'Agent IA',                   'Rédaction de messages et analyses intelligentes'],
        ];
        @endphp
        <div style="display:grid;gap:10px">
            @foreach($features as [$icon, $titre, $desc])
            <div style="display:flex;align-items:flex-start;gap:12px">
                <div style="width:34px;height:34px;border-radius:8px;background:#FFF7ED;color:#EA580C;
        ...
        <table class="table-immo">
            ...
            <tr>
                ...
                <td style="font-size:.75rem;color:#6B7280">
                    {{ $a->canal_paiement ? \App\Models\Paiement::CANAUX_MOBILE[$a->canal_paiement]['label'] ?? $a->canal_paiement : ($a->methode_paiement ?? '—') }}
                </td>
                <td>
...
<div id="paySection">
    <div class="card-immo" style="padding:0;overflow:hidden;position:sticky;top:76px">
        ...
        {{-- En-tête tarif --}}
        <div style="background:linear-gradient(135deg,#EA580C,#F97316);color:#fff;padding:24px 24px 20px">
            ...
            <div style="display:flex;align-items:flex-end;gap:6px">
                <span style="font-size:2.4rem;font-weight:800;line-height:1">{{ number_format($prix, 0, ',', ' ') }}</span>
                <span style="font-size:1rem;font-weight:600;opacity:.9;padding-bottom:4px">{{ $devSymbole }}</span>
        ...
        <div style="padding:22px">
            ...
            {{-- Champ téléphone MTN MoMo (API directe USSD) --}}
            ...
            {{-- Zone QR code (affiché si pas d'API directe mais QR disponible) --}}
            <div id="qrSection" style="display:none;margin-bottom:14px">
                <div style="text-align:center;padding:16px;background:#F9FAFB;border-radius:10px;border:1px dashed #D1D5DB">
                    ...
                    <div style="font-size:.72rem;color:#6B7280;margin-bottom:12px">
                        Envoyez exactement <strong>{{ number_format($prix, 0, ',', ' ') }} {{ $devSymbole }}</strong>
                    </div>
                    <div style="text-align:left">
                        ...
                        <div style="font-size:.68rem;color:#9CA3AF;margin-top:3px">
                            Copiez la référence/ID visible dans votre confirmation de paiement.
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="ussdWaiting" style="display:none;margin-bottom:14px">
                <div style="background:#FFFDE7;border:1.5px solid #FFCC00;border-radius:10px;padding:16px;text-align:center">
                    ...
                    <div style="font-size:.75rem;color:#78350F;margin-bottom:10px">
                        Ouvrez votre application MTN MoMo ou composez <strong>*133#</strong> pour confirmer le paiement de
                        <strong>{{ number_format($prix, 0, ',', ' ') }} {{ $devSymbole }}</strong>
                    </div>
...
const PRIX_LABEL   = '{{ number_format($prix, 0, ",", " ") }} {{ $devSymbole }}';
...
function selectCanal(code, color, bg) {
    ...
    // Réinitialiser toutes les zones spéciales
    resetZones();
    ...
    // Afficher la zone selon le mode
    ...
    btn.innerHTML = `<i class="bi bi-send-fill"></i> Confirmer le paiement — ${PRIX_LABEL}`;
}
...
async function payerAbonnement() {
    ...
    // Si mode QR — soumettre avec référence manuelle
    if (hasQr) {
    ...
    // Ajouter le téléphone si MTN API directe
    ...
    try {
        ...
        if (data.url) {
            // Redirection passerelle (Orange Money, Wave, CinetPay, Stripe)
            btn.innerHTML = '<span class="pulse"><i class="bi bi-arrow-right-circle"></i> Redirection…</span>';
            window.location.href = data.url;

        } else if (data.ussd) {
            // MTN MoMo USSD push — attente confirmation
            ...
            // Simulation ou activation immédiate
